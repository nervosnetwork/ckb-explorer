  ...
  module V1
    class AddressDaoTransactionsControllerTest < ActionDispatch::IntegrationTest
      ...
      test "should return corresponding ckb transactions with given address hash" do
        ...
        ckb_dao_transactions = presented_address.ckb_dao_transactions.order(block_timestamp: :desc).page(page).per(page_size)
      ...
      test "should return corresponding page's records when page is set and page_size is not set" do
        ...
        presented_address = AddressPresenter.new(address)
        address_dao_transactions = presented_address.ckb_dao_transactions.order(block_timestamp: :desc).page(page).per(page_size)
      ...
      def fake_dao_deposit_transaction(dao_cell_count, address)
        ...
        dao_cell_count.times do |number|
          ...
          create(:cell_output, ckb_transaction: generated_by, cell_index: number, tx_hash: "0x498315db9c7ba144cca74d2e9122ac9b3a3da1641b2975ae321d91ec34f1c0e3", generated_by: generated_by, block: block, capacity: 10**8 * 1000, cell_type: "dao", address: address)
        end
      end
    end
  end
