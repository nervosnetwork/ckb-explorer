module CsvExportable
  class ExportPortfolioTransactionsJob < BaseExporter
    def perform(args)
      ...
      ckb_transactions.find_in_batches(batch_size: 500, order: :desc) do |transactions|
        ...
        inputs = CellOutput.where(consumed_by_id: tx_ids, address_id: args[:address_ids])
        outputs = CellOutput.where(ckb_transaction_id: tx_ids, address_id: args[:address_ids])
