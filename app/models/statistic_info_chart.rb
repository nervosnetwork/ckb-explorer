class StatisticInfoChart
  ...
  def calculate_hash_rate
    ...
    if from == to
      ...
      unless hash_data_valid?(prev_cached_data.last(11))
        from = last_epoch0_block_number
        Rails.cache.delete_matched("hash_rate_chart_data_*")
      end
      to = max_block_number
      result =
  ...
  def hash_data_valid?(last11_hash_rate)
    return true if last11_hash_rate.blank?

    from = last11_hash_rate.first.number
    to = last11_hash_rate.last.number
    result =
      (from).step(to, 100).map do |number|
        blocks = Block.where("number <= ?", number).available.recent.includes(:uncle_blocks).limit(100)
        hash_rate_for_block(blocks, number)
      end

    last11_hash_rate == result
  end
