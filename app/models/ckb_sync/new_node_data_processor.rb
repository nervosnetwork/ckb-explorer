...
module CkbSync
  class NewNodeDataProcessor
    ...
    def build_new_dao_depositor_events!(local_block, new_dao_depositors, dao_contract)
      ...
      new_dao_depositors.each do |address_id, ckb_transaction_id_and_cell_index|
        new_dao_events_attributes << {
          block_id: local_block.id, ckb_transaction_id: ckb_transaction_id_and_cell_index[:ckb_transaction_id], cell_index: ckb_transaction_id_and_cell_index[:cell_index], address_id:, event_type: "new_dao_depositor",
      ...
      local_block.cell_outputs.nervos_dao_deposit.select(:id, :address_id, :capacity,
        ...
        dao_outputs.each do |dao_output|
          ...
          if address.dao_deposit.zero? && !new_dao_depositors.key?(address.id)
            new_dao_depositors[address.id] = { ckb_transaction_id: dao_output.ckb_transaction_id, cell_index: dao_output.cell_index }
          end
