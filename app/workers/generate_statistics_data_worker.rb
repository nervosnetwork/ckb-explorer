class GenerateStatisticsDataWorker
  ...
  def perform(block_id)
    ...
    block.cell_outputs.select(:id, :created_at, :data, :capacity, :lock_script_id, :type_script_id).find_in_batches do |cell_outputs|
      cell_outputs_attributes = []
      cell_outputs.each do |cell_output|
        cell_outputs_attributes << { id: cell_output.id, data_size: CKB::Utils.hex_to_bin(cell_output.data).bytesize,
