class UdtRegister
  ...
  def initialize
    namespace :migration do
      ...
      task :register_udt,
           [:mode, :args, :symbol, :full_name, :decimal, :description, :operator_website] => :environment do |_, args|
  ...
  def register_udts_from_url
    ...
    assets.each do |asset|
      args = {
        args: asset[:sudtArgs], symbol: asset[:symbol], full_name: asset[:name], decimal: asset[:decimal],
        icon_file: asset[:logoURI] }
      puts args
  ...
  def register_forcebridge_udt
    ...
    forcebridge_assets.each do |asset|
      ...
      args = {
        args: asset[:info][:shadow][:ident], symbol: asset[:info][:symbol], full_name: asset[:info][:name],
        decimal: asset[:info][:decimals], icon_file: asset[:info][:logoURI] }
      puts args
      type_script = build_udt_type_script(args)
      udt = Udt.find_by(type_hash: type_script.compute_hash)
      if udt.blank?
  ...
  def register_udt(args)
    ...
    ApplicationRecord.transaction do
      ...
      udt.update!(code_hash: type_script.code_hash, hash_type: type_script.hash_type, args: type_script.args,
                  symbol: args[:symbol], full_name: args[:full_name], decimal: args[:decimal], description: args[:description], operator_website: args[:operator_website], icon_file: args[:icon_file], issuer_address: issuer_address)
      udt.update!(published: true) if args[:icon_file].present?
      UdtAccount.where(udt_id: udt.id).update(symbol: udt.symbol, full_name: udt.full_name, decimal: udt.decimal,
