  ...
  task check_address_info: :environment do
    ...
    Address.where(last_updated_block_number: nil).find_each do |address|
      if address.last_updated_block_number.nil?
        cell_output = CellOutput.where(address_id: address.id).order("block_timestamp desc").limit(1)
        if cell_output.present?
          address.last_updated_block_number = cell_output.first.block.number
